...
// Begin ULCC hack
// We want to try to authenticate via CAS to achieve transparent single sign on, but only if nothing
// was submitted via the login form
if (!$USER->is_logged_in() && !isset($_POST['login_username'])) {

    safe_require('auth', 'casdb');

    $casauths = get_records_array('auth_instance', 'instancename', 'casdb');
    $casplugin = false;
    if ($casauths) {
//        print_r($casauths);
//        die();
        $casplugin = AuthFactory::create($casauths[0]->id);
        /**
         * @var $casplugin AuthCasDb
         */
        $casplugin->attempt_auth();
    }
}
// End ULCC hack
...
if (!defined('INSTALLER')) {
    // This will check for the submitted login form and process it if appropriate, leaving the user logged in.
    auth_setup();
}
